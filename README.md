...
We currently support DynamoDB's **BatchWriteItem** via Ecto's `Repo.insert_all()`. DynamoDB enforces a limit of 25 records maximum per batch write, but we allow for an unlimited number of records by chunking large groups into batches. Each chunk will return a nested map structure with information about any unprocessed items. In the debug logs for this operation, each nested map will be reported separately, accumulating in a running list - for example, `[{"UnprocessedItems":{"table_name":[{"PutRequest":"info here..."}]}},{"UnprocessedItems":{"table_name":[{"PutRequest":"other info here..."}]}}]` will be logged after two batches. In the final info log of this operation, the accumulated results of the batches will be 'prettified', logging `{"UnprocessedItems":{"table":[{"PutRequest":"info here..."},{"PutRequest":"other info here..."}]}}`.
